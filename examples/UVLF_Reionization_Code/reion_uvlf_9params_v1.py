...
def cal_chisq_tau_elec(model_tau):
    """
    ...
    return ((model_tau - tau_e_obs) / sigma_tau_e_obs) ** 2


def cal_chisq_QHI(z_arr, QHII_arr):
    """
    ...
    for i, z_obs in enumerate(obs_zarr):
        ...
        mean_obs = obs_xHI_arr[i]
        sigma_hi = hi_sigmaxHI_arr[i]
        sigma_lo = lo_sigmaxHI_arr[i]
        ...
        if (model >= mean_obs):
            chisq += ((mean_obs - model) / sigma_hi) ** 2
        else:
            chisq += ((mean_obs - model) / sigma_lo) ** 2

    return chisq


def cal_chisq_eachUVLF(log10_fstar10_by_cstar, alpha_star, log10Mcrit, QHI_zval, zval, obsdatafile):
    """
    ...
    for i, obsMUV in enumerate(obsMUV_cen):
        ...
        mean_obs = obsPhiUV[i]
        sigma_hi = obsPhiUV_hierr[i]
        sigma_lo = obsPhiUV_loerr[i]
        model = modelUVLF_total[i]

        if (model >= mean_obs):
            chisq_eachUVLF += ((mean_obs - model) / sigma_hi) ** 2
        else:
            chisq_eachUVLF += ((mean_obs - model) / sigma_lo) ** 2

    return chisq_eachUVLF


def cal_chisq_allUVLF(QHI_UVLF_arr, log10Mcrit, lsum, ldiff, l2, l3, asum, adiff, a2, a3):
    """
    ...
    for i, zdata in enumerate(zUVLF_arr):
        ...
        chisq_allUVLF = chisq_allUVLF + cal_chisq_eachUVLF(log10fstar10_by_cstar,alpha_star, log10Mcrit, QHI, zdata, obsdatafile)

    # outside for-loop
    return chisq_allUVLF
...
def log_likelihood(lsum, ldiff, l2, l3, asum, adiff,  log10_fesc10, alpha_esc, log10Mcrit):
    ...
    model_chisq_allUVLF = cal_chisq_allUVLF(model_QHI_UVLF_arr,log10Mcrit,lsum, ldiff, l2, l3, asum, adiff, a2, a3)
    
    
   
    model_chisq_QHI = cal_chisq_QHI(model_z_arr, model_QHII_arr)
    
    
    model_chisq_tau_elec = cal_chisq_tau_elec (model_tau_value)
    
    
    print("model_chisq_allUVLF = ", model_chisq_allUVLF)
    print("model_chisq_QHI = ", model_chisq_QHI)
    print("model_chisq_tau_elec = ", model_chisq_tau_elec)
    
    
    chisq_total = model_chisq_QHI + model_chisq_tau_elec + model_chisq_allUVLF
...
derived_name_arr = ["tau_e","twofone","twoaone","l0","l1","a0","a1"]
